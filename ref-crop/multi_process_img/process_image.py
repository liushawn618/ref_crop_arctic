...
parser.add_argument("--use_old_folder", action="store_true")
parser.add_argument("--save_mode", type=str, choices=["ln", "direct", "no", "clear"], default="direct",
...
parser.add_argument("--save_position", action="store_true")
...
if __name__ == "__main__":
    if args.use_old_folder:
        print(f"{seq_name} use old folder")
    else:
    ...
    for png_file_name in tqdm.tqdm(png_list, desc=f"{seq_name}"):
        ...
        position_info_l[png_file_name] = l_border
        position_info_r[png_file_name] = r_border
        position_info_o[png_file_name] = o_border
    ...
    if args.save_position:
        with open(os.path.join(l_save_path, "position_info.json"), "w") as f:
            json.dump(position_info_l, f)
            print(f"{seq_name} save position_info_l to {os.path.join(l_save_path, 'position_info.json')}")
        with open(os.path.join(r_save_path, "position_info.json"), "w") as f:
            json.dump(position_info_r, f)
            print(f"{seq_name} save position_info_r to {os.path.join(r_save_path, 'position_info.json')}")
        with open(os.path.join(o_save_path, "position_info.json"), "w") as f:
            json.dump(position_info_o, f)
            print(f"{seq_name} save position_info_o to {os.path.join(o_save_path, 'position_info.json')}")
