...
from src.models.ref_crop_arctic_sf.wrapped_hmr import WrappedHandHMR
from src.models.ref_crop_arctic_sf.wrapped_hmr import WrappedObjectHMR


from src.models.config import ModelConfig

from src.datasets.ref_crop_arctic_dataset import BOX_POSI_VEC_LEN
...
class RefCropArcticSF(nn.Module):
    def __init__(self, backbone, focal_length, img_res, args):
        ...
        self.head_r = WrappedHandHMR(feat_dim + BOX_POSI_VEC_LEN, is_rhand=True, n_iter=3)
        self.head_l = WrappedHandHMR(feat_dim + BOX_POSI_VEC_LEN, is_rhand=False, n_iter=3)
        self.head_o = WrappedObjectHMR(feat_dim + BOX_POSI_VEC_LEN, n_iter=3)
    ...
    def forward(self, inputs, meta_info):
        ...
        posi_vec_l = inputs["ref_img_l_posi_vec"]
        posi_vec_r = inputs["ref_img_r_posi_vec"]
        posi_vec_obj = inputs["ref_img_o_posi_vec"]
        ...
        hmr_output_r_ref = self.head_r.forward(features_r, posi_vec_r)
        hmr_output_l_ref = self.head_l.forward(features_l, posi_vec_l)
        hmr_output_obj_ref = self.head_o.forward(features_obj, posi_vec_obj)
